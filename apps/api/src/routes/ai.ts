...
router.post("/edit", requireAuth(), async (req: Request, res: Response) => {
  try {
    ...
    if (cleanPrompt.includes("premium") || cleanPrompt.includes("luxury") || cleanPrompt.includes("classy") || cleanPrompt.includes("sophisticated")) {
      ...
      const heroSection = modified.pages[0].sections.find((s: any) => s.type === "hero");
      if (heroSection) {
    ...
    // 2. Add Cake Gallery / Add Gallery
      ...
      const hasGallery = page.sections.some((s: any) => s.type === "gallery");
    ...
    // 5. Add customer testimonials
      ...
      const page = modified.pages[0];
      const hasTestimonials = page.sections.some((s: any) => s.type === "testimonials");
