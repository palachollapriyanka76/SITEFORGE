...
export const INDUSTRY_ALLOWED_KEYWORDS = {
...
export const INDUSTRY_DISALLOWED_KEYWORDS = {
...
export function getImageCategory(url) {
    const id = getImageIdFromUrl(url);
    return IMAGE_CATEGORIES[id] || "unknown";
}
export function getProductCategory(name) {
    const n = name.toLowerCase();
    if (n.includes("cupcake"))
        return "cupcake";
    if (n.includes("croissant"))
        return "croissant";
    if (n.includes("cookie"))
        return "cookies";
    if (n.includes("cake") || n.includes("gateau"))
        return "cake";
    if (n.includes("bread") || n.includes("sourdough"))
        return "bread";
    if (n.includes("pastry") || n.includes("pastries"))
        return "pastry";
    if (n.includes("donut") || n.includes("donuts"))
        return "donut";
    if (n.includes("muffin") || n.includes("muffins"))
        return "muffin";
    if (n.includes("pizza"))
        return "pizza";
    ...
    if (n.includes("paneer"))
        return "paneer";
    if (n.includes("chicken") || n.includes("tikka") || n.includes("kabab"))
        return "chicken";
    if (n.includes("hair"))
        return "hair";
    if (n.includes("makeup") || n.includes("bridal") || n.includes("makeover"))
        return "makeup";
    if (n.includes("nail"))
        return "nail";
    if (n.includes("spa") || n.includes("massage"))
        return "spa";
    ...
    if (n.includes("phone") || n.includes("mobile"))
        return "phone";
    if (n.includes("headphone") || n.includes("audio") || n.includes("anc"))
        return "headphones";
    if (n.includes("watch"))
        return "watch";
    ...
    if (n.includes("tablet") || n.includes("ipad"))
        return "tablet";
    if (n.includes("weight") || n.includes("barbell") || n.includes("dumbbell") || n.includes("plates"))
        return "weights";
    if (n.includes("treadmill") || n.includes("running"))
        return "treadmill";
    if (n.includes("yoga"))
        return "yoga";
    if (n.includes("stethoscope"))
        return "stethoscope";
    if (n.includes("dentist") || n.includes("dental") || n.includes("teeth") || n.includes("tooth"))
        return "dentist";
    if (n.includes("suit"))
        return "suit";
    if (n.includes("sew") || n.includes("stitching") || n.includes("tailor"))
        return "sewing";
    return "other";
}
export function extractPriceNumber(priceStr) {
    const clean = priceStr.replace(/[^0-9]/g, "");
    return parseInt(clean, 10) || 0;
}
export function runQualityAudit(websiteJson, businessData) {
    const logs = [];
    ...
    if (type.includes("bakery") || type.includes("cake") || type.includes("sweet"))
        industry = "bakery";
    else if (type.includes("rest") || type.includes("cafe") || type.includes("food") || type.includes("dine"))
        industry = "restaurant";
    else if (type.includes("salon") || type.includes("spa") || type.includes("hair") || type.includes("beauty"))
        industry = "salon";
    else if (type.includes("fashion") || type.includes("clothing") || type.includes("boutique") || type.includes("wear"))
        industry = "fashion";
    else if (type.includes("elect") || type.includes("phone") || type.includes("gadg"))
        industry = "electronics";
    else if (type.includes("gym") || type.includes("fit"))
        industry = "gym";
    else if (type.includes("clinic") || type.includes("medical") || type.includes("doctor") || type.includes("health"))
        industry = "medical";
            ...
            if (sec.content) {
                ...
                Object.entries(sec.content).forEach(([key, val]) => {
                    if (typeof val === "string") {
                        if (placeholderRegex.test(val))
                            placeholderCount++;
                        if (val.toLowerCase().includes("premium quality sourced from"))
                            boilerplateCount++;
                    }
                });
                ...
                if (Array.isArray(cardsList)) {
                    cardsList.forEach((prod) => {
                        ...
                        if (prodName)
                            titlesList.push(prodName.trim().toLowerCase());
                        if (prodDesc)
                            descriptionsList.push(prodDesc.trim().toLowerCase());
                        if (prodPrice)
                            pricesList.push(prodPrice.trim().toLowerCase());
    ...
    if (titlesList.length > uniqueTitles)
        uniquenessDeductions += 20;
    if (descriptionsList.length > uniqueDescs)
        uniquenessDeductions += 20;
    if (pricesList.length > uniquePrices)
        uniquenessDeductions += 20;
...
export function passesIndustryRecognitionTest(websiteJson, industry) {
    const allowed = INDUSTRY_ALLOWED_KEYWORDS[industry] || [];
    if (allowed.length === 0)
        return true; // generic/unknown passes by default
    const textPool = [
        ...
        ...websiteJson.pages.flatMap(p => p.sections.flatMap(s => {
            ...
            if (s.content) {
                if (s.content.title)
                    parts.push(s.content.title);
                if (s.content.subtitle)
                    parts.push(s.content.subtitle);
            }
            return parts;
        }))
    ].join(" ").toLowerCase();
