...
export default function StandalonePreviewPage({ params }) {
  ...
  if (!templateJson) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-zinc-950 text-white font-mono gap-3">
        <Loader2 className="w-8 h-8 animate-spin text-[#84A98C]" />
        <span>Compiling website preview...</span>
      </div>
    );
  }

  return (
    <div className="w-full min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans select-none relative overflow-x-hidden">
      
      {/* Floating Preview Toolbar */}
      <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-zinc-900/90 border border-zinc-805 text-white rounded-full px-5 py-2 flex items-center gap-6 shadow-2xl backdrop-blur-md shrink-0">
        <BackButton fallback="/templates" label="Back to Templates" />
        ...
        <div className="flex bg-zinc-950 border border-zinc-850 rounded-lg p-0.5">
          <button
            ...
            className={`p-1.5 rounded-md transition-all ${viewportMode === "desktop" ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"}`}
            title="Desktop View"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-all ${viewportMode === "tablet" ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"}`}
            title="Tablet View"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-all ${viewportMode === "mobile" ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"}`}
            title="Mobile View"
          >
        ...
        {/* Simulated Website Pages Container Selector */}
        <div className="flex bg-zinc-950 border border-zinc-850 rounded-lg p-0.5">
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-[#52796F] hover:bg-[#354F52] text-white text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full transition-colors shadow-lg disabled:opacity-50"
        >
          {isDeploying ? (
            <>
            ...
            <>
              Use This Template <ArrowUpRight className="h-3 w-3" />
            </>
          )}
        </button>
      </div>
      ...
      <main className="flex-1 w-full bg-zinc-950 flex justify-center items-start pt-24 pb-12 px-4 sm:px-8">
        <div
          className="bg-white text-zinc-800 shadow-2xl transition-all duration-300 overflow-hidden flex flex-col min-h-full border border-zinc-800 rounded-2xl"
          style={{
        ...
        >
          {/* Simple Top Browser Chrome Header */}
          ...
          {/* Simulated Website Pages Header (for responsive/mobile layouts) */}
    ...
    <div className="space-y-4">
      {(faqs || []).map((faq, idx) => {
        ...
        return (
          <div 
            key={idx} 
            className="p-5 transition-all text-left border border-zinc-105 bg-zinc-50/50 rounded-2xl cursor-pointer"
...
function RenderSection({ sec, theme, onAnchorClick, triggerToast, onGalleryImageClick }) {
  ...
  return (
    <div id={sec.id} className="relative group">
      {/* 1. HERO */}
      {sec.type === "hero" && (
        <section className="py-24 px-8 text-center text-white relative overflow-hidden bg-zinc-905" style={{ minHeight: "450px" }}>
              ...
              {content.highlights && (
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {content.highlights.map((hl, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2.5 text-[10.5px] text-zinc-800 font-bold bg-[#84A98C]/8 border border-[#84A98C]/15 px-3 py-2.5 rounded-2xl shadow-sm hover:shadow-md hover:bg-[#84A98C]/15 transition-all duration-205">
                        {!hasEmoji && (
                ...
                >
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between gap-3 text-left">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-zinc-505 leading-relaxed">{prod.description}</p>
                    </div>
                    <button 
              ...
              {(content.testimonials || []).map((test, idx) => (
                ...
                >
                  <div className="space-y-3">
                    ...
                    <p className="text-[10.5px] text-zinc-650 italic leading-relaxed">"{test.content}"</p>
                  </div>
                  <div className="flex items-center gap-3 border-t border-zinc-100 pt-3">
                    {test.avatar && (
                      <div className="h-7 w-7 rounded-full overflow-hidden bg-zinc-205">
              ...
              <form onSubmit={(e) => {
                ...
                {(content.fields || []).map((fld, idx) => (
                  <div key={idx} className="space-y-1.5">
                    <label className="text-[8.5px] font-bold text-zinc-505 uppercase tracking-wider block">{fld.label}</label>
                    {fld.type === "select" ? (
                      <select className="w-full h-9 px-3 border border-zinc-250 text-[10px] outline-none bg-zinc-50 font-sans" style={{ borderRadius: theme.style === "minimal" ? "0px" : "8px" }}>
                        {(fld.options || []).map((opt, oidx) => (
                          <option key={oidx}>{opt}</option>
                        ))}
                      </select>
                    ) : (
                      <input required type={fld.type} placeholder={fld.placeholder} className="w-full h-9 px-3 border border-zinc-250 text-[10px] outline-none bg-white font-sans" style={{ borderRadius: theme.style === "minimal" ? "0px" : "8px" }} />
                    )}
                  </div>
                ))}
            ...
            <div className="space-y-8 text-left">
              {(content.categories || []).map((cat, idx) => (
                <div key={idx} className="space-y-4">
                  <span className="text-xs font-black text-zinc-905 block border-l-4 pl-3.5 uppercase tracking-widest" style={{ borderColor: theme.primaryColor }}>
                    {cat.name}
                  </span>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(cat.items || []).map((item, iidx) => (
                      <div 
                        key={iidx} 
                        className="p-4 flex flex-col justify-between gap-2.5 border border-zinc-100 bg-zinc-50/50 rounded-2xl cursor-pointer hover:shadow-md transition-all duration-305"
                ...
                >
                  ...
                  <div className="space-y-4 text-left">
                    <div className="space-y-1">
                      ...
                      <span className="text-[8px] font-bold text-zinc-450 uppercase tracking-widest block">{plan.duration}</span>
                    </div>
              ...
              {(content.stories || []).map((story, idx) => (
                ...
                >
                  ...
                  <div className="space-y-2.5 text-[10.5px]">
                    <div>
                      <strong className="text-zinc-550 block uppercase text-[8px] tracking-wider">Before State:</strong>
                      <span className="text-zinc-650">{story.before}</span>
                    </div>
                    <div>
                      <strong className="text-zinc-550 block uppercase text-[8px] tracking-wider">Transformation & Result:</strong>
                      <span className="text-zinc-850 font-bold" style={{ color: theme.primaryColor }}>{story.result}</span>
                    </div>
                    <div>
                      <strong className="text-zinc-550 block uppercase text-[8px] tracking-wider">After State:</strong>
